...
public class Patient4_Controller implements Initializable {
    @FXML
    private javafx.scene.control.TableView<LabRequestClass> LabResultTable;
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        try {
            ResultSet rs = DB_Connection.select("SELECT * FROM diagnosis, doctor, patient, patients_history where patients_history.P_ID = Patient.P_ID and Patient.P_ID = diagnosis.P_ID and doctor.D_ID = diagnosis.D_ID and Patient.P_ID ='"+patientID+"'");
            while (rs.next()) {
            ...
            ResultSet rs2 = DB_Connection.select("SELECT * FROM lab_test, lab_request, patient, patients_history where Patients_history.P_ID = patient.P_ID and patient.P_ID = lab_request.P_ID and lab_request.Test_code = lab_test.Test_code and Patient.P_ID ='"+patientID+"'");
            while (rs2.next()) {
    ...
    public static void search(TableView<DiagnosisClass> table, TextField search_tf, ObservableList<DiagnosisClass> list){
        ...
        search_tf.textProperty().addListener((observable, newValue, oldValue) -> {
            filterList.setPredicate(diagnosis ->  {
                ...
                if (String.valueOf(diagnosis.getPatientId()).contains(searchKeyword)) {
                ...
                else if (diagnosis.getDoctorName().toLowerCase().contains(searchKeyword)) {

                    return true;

                }
                else if (diagnosis.getDoctorSpecialization().toLowerCase().contains(searchKeyword)) {

                    return true;

                } else if (diagnosis.getDiagnosis_date().toString().contains(searchKeyword)) {

                    return true;
                }
                else if (diagnosis.getDiagnosis_Time().toString().contains(searchKeyword)) {

                    return true;
                }
                else if (diagnosis.getDiagnose().toLowerCase().contains(searchKeyword)) {

                    return true;
                }
                else if (diagnosis.getTreatment().toLowerCase().contains(searchKeyword)) {
    ...
    public static void search2(TableView<LabRequestClass> table, TextField search_tf, ObservableList<LabRequestClass> list){
        ...
        search_tf.textProperty().addListener((observable, newValue, oldValue) -> {
            filterList.setPredicate(lab_request ->  {
                ...
                if (String.valueOf(lab_request.getPatientID()).contains(searchKeyword)) {

                    return true; //means we find match in patient ID
                }
                else if (lab_request.getPatientName().toLowerCase().contains(searchKeyword)) {

                    return true;

                }
                else if (lab_request.getLAB_date().toString().contains(searchKeyword)) {

                    return true;
                }
                else if (lab_request.getStartTime().toString().contains(searchKeyword)) {

                    return true;
                }
                else if (lab_request.getTestName().toLowerCase().contains(searchKeyword)) {
